...
```bash
gh api -X POST \
  /repos/SemiAnalysisAI/InferenceX-Private-TPU/actions/workflows/e2e-tests.yml/dispatches \
  -f ref='main' \
...
```bash
RUN_ID=$(gh run list --repo SemiAnalysisAI/InferenceX-Private-TPU --workflow e2e-tests.yml \
  --event workflow_dispatch --limit 1 --json databaseId --jq '.[0].databaseId')
gh run watch "$RUN_ID" --repo SemiAnalysisAI/InferenceX-Private-TPU --exit-status   # block, non-zero on failure
gh run view "$RUN_ID" --repo SemiAnalysisAI/InferenceX-Private-TPU --log-failed     # inspect failures
gh run cancel "$RUN_ID" --repo SemiAnalysisAI/InferenceX-Private-TPU                # cancel
```
...
```bash
gh api /repos/SemiAnalysisAI/InferenceX-Private-TPU/actions/runs/<RUN_ID>/artifacts --jq '.artifacts[].name'
gh run download <RUN_ID> --repo SemiAnalysisAI/InferenceX-Private-TPU -n results_bmk -D ./results
```
