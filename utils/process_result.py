...
if total_token_throughput <= 0 or output_throughput <= 0:
    raise ValueError(
        "Benchmark produced non-positive throughput; no successful generation "
...
if is_multinode:
    ...
    multi_node_data = {
        ...
        'tput_per_gpu': total_token_throughput / (total_gpus / chip_divisor),
        'output_tput_per_gpu': output_throughput / (output_tput_denominator / chip_divisor),
        'input_tput_per_gpu': (total_token_throughput - output_throughput) / (prefill_gpus / chip_divisor),
    }
    if prefill_hardware:
