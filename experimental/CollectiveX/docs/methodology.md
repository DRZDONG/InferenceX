...
Runtime `vendor` is registry metadata rather than a CUDA/HIP inference. It accepts any normalized
vendor identifier; the separate `runtime` registry field selects the currently implemented CUDA or
HIP execution path. This keeps result identity open to private non-AMD/NVIDIA systems without
pretending that a new accelerator runtime can reuse an incompatible backend or launcher.
