    ...
    def test_every_platform_entry_is_complete_and_typed(self) -> None:
        ...
        for name, entry in platforms.items():
            with self.subTest(sku=name):
                for field in (
                    "arch", "vendor", "runtime", "product", "image", "image_platform",
...
class ConfigTests(unittest.TestCase):
    def test_operator_config_emits_allowlisted_values(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertIn(b"COLLX_VENDOR\0nvidia\0", payload)
            self.assertIn(b"COLLX_RUNTIME\0cuda\0", payload)
    ...
    def test_operator_config_accepts_non_amd_nvidia_vendor(self) -> None:
        platforms = {
            "www": {
                "image": "registry.example/www:latest",
                "image_platform": "linux/amd64",
                "vendor": "www",
                "runtime": "cuda",
            }
        }
        with mock.patch.object(config, "_platforms", return_value=platforms):
            payload = self._emit_registry_only("www")
        self.assertIn(b"COLLX_VENDOR\0www\0", payload)
        self.assertIn(b"COLLX_RUNTIME\0cuda\0", payload)


class RuntimeMetadataTests(unittest.TestCase):
    def test_vendor_metadata_is_independent_of_runtime(self) -> None:
        torch = types.SimpleNamespace(
            __version__="test",
            version=types.SimpleNamespace(cuda="13.0", hip=None),
        )
        with mock.patch.object(run_ep, "_loaded_collective_version", return_value="2.30.4"):
            runtime = run_ep._runtime_info(torch, vendor="www", runtime_kind="cuda")
        self.assertEqual(runtime["vendor"], "www")
        self.assertEqual(runtime["accelerator_runtime"], "13.0")
        self.assertEqual(runtime["collective_library"], {"kind": "nccl", "version": "2.30.4"})
