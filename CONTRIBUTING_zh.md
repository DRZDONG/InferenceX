...
- 保持模板的开头语句原样不变（必须保留英文原文）：
  ...
  我们的 CI 验证工作流 [`codeowner-signoff-verify.yml`](https://github.com/SemiAnalysisAI/InferenceX-Private-TPU/blob/main/.github/workflows/codeowner-signoff-verify.yml) 正是通过这句话触发的。**如果你的批准评论没有遵循清单模板 — 包括这句话 — 签署验证 CI 将完全不会触发**，你的签署也不会计入合并要求。
