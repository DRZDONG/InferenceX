...
TARGET_JSON=$(gh api \
  "repos/SemiAnalysisAI/InferenceX-Private-TPU/actions/runs/$TARGET_RUN_ID")
ORIGINAL_MERGE_SHA=$(jq -r .head_sha <<<"$TARGET_JSON")
ORIGINAL_PR=$(gh api \
  "repos/SemiAnalysisAI/InferenceX-Private-TPU/commits/$ORIGINAL_MERGE_SHA/pulls" \
...
gh run list --repo SemiAnalysisAI/InferenceX-Private-TPU \
...
skipped, `trigger-ingest` still fires `always()` and lands a *bogus*
...
SOURCE_JSON=$(gh api \
  "repos/SemiAnalysisAI/InferenceX-Private-TPU/actions/runs/$SOURCE_RUN_ID")
...
SOURCE_ARTIFACTS=$(gh api \
  "repos/SemiAnalysisAI/InferenceX-Private-TPU/actions/runs/$SOURCE_RUN_ID/artifacts?per_page=100" \
...
```bash
gh api \
  "repos/SemiAnalysisAI/InferenceX-Private-TPU/pulls/$ORIGINAL_PR/commits" \
  --paginate --jq '.[].sha' |
...
```bash
gh api \
  "repos/SemiAnalysisAI/InferenceX-Private-TPU/pulls/$RECOVERY_PR/commits" \
  --paginate --jq '.[].sha' |
  grep -Fx "$SOURCE_HEAD_SHA"
...
gh run watch "$RECOVERY_RUN_ID" \
  --repo SemiAnalysisAI/InferenceX-Private-TPU --exit-status
```
...
download, database ingest, run overrides, database verification,
