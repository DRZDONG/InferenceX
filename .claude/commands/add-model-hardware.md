...
gh pr create --repo SemiAnalysisAI/InferenceX-Private-TPU --base main \
  --title "[Klaud Cold] <key>: day-zero <MODEL> <SKU> recipe" --body "<summary>"
# fill perf-changelog pr-link with the real URL → commit → push
gh api -X POST repos/SemiAnalysisAI/InferenceX-Private-TPU/issues/<PR>/labels -f "labels[]=full-sweep-fail-fast" --jq '.[].name'
```
