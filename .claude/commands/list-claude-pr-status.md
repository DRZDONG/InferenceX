...
while IFS=$'\t' read -r pr title; do
  rollup=$(gh pr view "$pr" --repo SemiAnalysisAI/InferenceX-Private-TPU --json statusCheckRollup)
...
```bash
{
  ...
  awk -F'\t' '$2 == "RUNNING"' /tmp/claude_pr_status.tsv
                ...
                {printf "| [#%s](https://github.com/SemiAnalysisAI/InferenceX-Private-TPU/pull/%s) | %s | `%s` | %s |\n", $1, $1, $2, $3, $4}'
```
