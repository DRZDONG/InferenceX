...
while IFS=$'\t' read -r pr branch title; do
  qualifies=$(gh pr view "$pr" --repo SemiAnalysisAI/InferenceX-Private-TPU --json statusCheckRollup --jq '
...
```bash
HEAD_SHA=$(gh pr view "$PR" --repo SemiAnalysisAI/InferenceX-Private-TPU --json headRefOid --jq .headRefOid)

# Find the most recent Run Sweep run for this commit
RUN_ID=$(gh run list --repo SemiAnalysisAI/InferenceX-Private-TPU --workflow "Run Sweep" \
  --commit "$HEAD_SHA" --limit 1 --json databaseId --jq '.[0].databaseId')

# Failing job IDs + names
gh run view "$RUN_ID" --repo SemiAnalysisAI/InferenceX-Private-TPU --json jobs \
  --jq '.jobs[] | select(.conclusion == "failure") | "\(.databaseId)\t\(.name)"' \
  > /tmp/failed_jobs_$PR.tsv

# Failure-only log dump (uses --log-failed)
gh run view "$RUN_ID" --repo SemiAnalysisAI/InferenceX-Private-TPU --log-failed \
...
| PR | Action | Note |
|---|---|---|
| [#NNNN](https://github.com/SemiAnalysisAI/InferenceX-Private-TPU/pull/NNNN) | fix pushed (`<sha>`) | one-line diagnosis |
| [#NNNN](https://github.com/SemiAnalysisAI/InferenceX-Private-TPU/pull/NNNN) | skipped | reason (flake / unclear / too large) |
