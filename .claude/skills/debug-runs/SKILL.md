...
RUN_ID=$(gh run list --repo SemiAnalysisAI/InferenceX-Private-TPU --workflow "Run Sweep" --commit "$HEAD_SHA" --limit 1 --json databaseId --jq '.[0].databaseId')
gh run watch "$RUN_ID" --repo SemiAnalysisAI/InferenceX-Private-TPU --interval 30
```
...
gh run view "$RUN_ID" --repo SemiAnalysisAI/InferenceX-Private-TPU --log-failed > /tmp/sweep_failed.txt
```
...
```bash
gh run rerun "$RUN_ID" --repo SemiAnalysisAI/InferenceX-Private-TPU --failed
```
